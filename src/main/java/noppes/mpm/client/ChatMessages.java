...
public class ChatMessages
{
  ...
  public static void test() { test("<Sirnoppes01> :)", "Sirnoppes01: :)");
  ...
  private static void test(String toParse, String result) { for (Pattern pattern : patterns) {
      Matcher m = pattern.matcher(toParse);
      if (m.find()) {
        String username = m.group(1);
        String message = m.group(2);
  ...
  private Map<Long, TextBlockClient> getMessages() {
    ...
    long time = System.currentTimeMillis();
    for (Iterator localIterator = this.messages.keySet().iterator(); localIterator.hasNext();) { long timestamp = ((Long)localIterator.next()).longValue();
