...
public class GuiModelLegs extends GuiModelInterface
{
  ...
  private final String[] arrTail = { "gui.no", "Player", "Player Dragon", "Cat", "Wolf", "Horse", "Dragon", "Squirrel", "Fin", "Rodent" };
  
  public GuiModelLegs(GuiScreen parent) { this.parent = parent;
    this.xOffset = 60;
  }
  ...
  public void initGui()
  {
    ...
    y += 22;addButton(new GuiNpcButton(1, this.guiLeft + 50, y, 70, 20, this.arrLegs, getNagaIndex(this.playerdata.legParts)));
    ...
    y += 22;addButton(new GuiNpcButton(2, this.guiLeft + 50, y, 70, 20, this.arrTail, getTailIndex(tail)));
  ...
  private int getNagaIndex(ModelPartData data) { if ((!data.playerTexture) && (data.type == 1))
      return 5;
    if (data.type == 4)
      return 6;
    if (data.type == 5)
